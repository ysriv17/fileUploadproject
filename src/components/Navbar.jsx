...
const Navbar = () => {
  ...
  return (
    <nav
      className="sm:px-16 px-6 w-full flex items-center py-5 fixed top-0 z-20 bg-gradient">
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to="/" className="flex items-center gap-2" onClick={() => { setActivity(""); window.scrollTo(0, 0); }}>
          <img src={logo} alt="logo" className='w -9 h-9 object-contain' />
          <p className='text-white text-[18px] font-bold cursor-pointer'>RESUMcheck</p>
        </Link>
